...
def get_log_likelihood(logprobs, actions=None, mask=None, return_sum: bool = True):
    ...
    assert (
        logprobs > -1000
    ).data.all(), f"Logprobs should not be -inf, check sampling procedure! {debugging()}"
...
def process_logits(
    ...
    # assert tanh_clipping > 0, "tanh_clipping should be > 0"
    if tanh_clipping > 0:
    ...
    assert temperature == 1.0, "Temperature scaling is not implemented yet"
    assert not torch.isnan(logits).any(), "Logits contain NaN values"
    assert not torch.isposinf(logits).any(), "Logits contain +Inf values"
